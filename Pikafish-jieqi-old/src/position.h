...
public:
  ...
  // The captured piece is deliberately kept in its dark encoding.  A game
  // manager may expose its type to the side that made the capture without
  // changing the public board representation.
  Piece captured_dark_piece() const;
  PieceType captured_dark_type() const;

  // Doing and undoing moves
  bool getDark(StateInfo& newSt, int& typecount, bool& isDarkDepth);
  void setDark();
  Piece do_flip(Square s, Piece pc);
...
private:
  ...
  void recompute_material_and_psq();
...
inline Piece Position::captured_dark_piece() const {
  return Darkof(st->capturedPiece) == UNKNOWN ? st->capturedPiece : NO_PIECE;
}

inline PieceType Position::captured_dark_type() const {
  Piece p = captured_dark_piece();
  return p == NO_PIECE ? NO_PIECE_TYPE : type_of(p);
}
