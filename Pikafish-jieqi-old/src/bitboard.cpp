...
void Bitboards::init() {
  ...
  for (Square s1 = SQ_A0; s1 <= SQ_I9; ++s1)
  {
      ...
      // Only generate pseudo attacks in the palace squares for the king.
      if (Palace & s1) {
      ...
      // In jieqi, an unrevealed advisor square uses advisor geometry, and
      // advisors are allowed to cross the river.  ADVISOR_B is the hidden
      // square type, so keep its diagonal movement unrestricted as well.
      for (int step : { NORTH_WEST, NORTH_EAST, SOUTH_WEST, SOUTH_EAST })
