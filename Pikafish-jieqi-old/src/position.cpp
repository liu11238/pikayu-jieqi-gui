...
namespace Zobrist {
  ...
  // A dark-piece identity can be indexed by its remaining ordinal.  The old
  // five-element array was indexed by values up to six and caused an out of
  // bounds access during position setup/search.
  Key psqDark[PIECE_NB][16];
  Key side;
}
...
constexpr PieceType DarkCandidateTypes[] = {
...
void Position::set_state(StateInfo* si) const {
  ...
  si->darkDepth = 0;
  si->darkTypes = 0;
  si->darkPiece = NO_PIECE;
  si->darkSquare = SQ_NONE;
  si->darkTypeIndex = 0;
...
bool Position::legal(Move m) const {
  ...
  // An unrevealed piece on an advisor square must stay within the palace.
  // ADVISOR_B is the old engine's hidden advisor-square piece type.
  if (type_of(moved_piece(m)) == ADVISOR_B && isDark(from) && !(Palace & to))
      return false;
...
Piece Position::do_flip(Square s, Piece pc) {
    assert(is_ok(s) && isDark(s));
    assert(pc != NO_PIECE && Darkof(pc) == KNOWN);

    Piece fromPc = piece_on(s);
    Color us = color_of(fromPc);
    assert(color_of(pc) == us);

    PieceType pt = type_of(pc);
    int count = restPieces[us].countType(pt);
    assert(count > 0);

    // Replace the hidden piece by the candidate identity.
    remove_piece(s);
    restPieces[us].pop_back(pt);
    put_piece(pc, s);

    // The pool average changed, so every other unknown piece's value and
    // positional score changed as well.  Recompute both instead of trying to
    // patch them incrementally.
    recompute_material_and_psq();
    st->key ^= Zobrist::psq[fromPc][s] ^ Zobrist::psq[pc][s]
             ^ Zobrist::psqDark[pc][count - 1];
...
void Position::undo_flip(Square s, Piece fromPc) {
    assert(is_ok(s) && fromPc != NO_PIECE && Darkof(fromPc) == UNKNOWN);

    Piece pc = piece_on(s);
    Color us = color_of(pc);
    PieceType pt = type_of(pc);
    assert(Darkof(pc) == KNOWN);

    remove_piece(s);
    restPieces[us].push_back(pc);
    int restoredCount = restPieces[us].countType(pt);
    put_piece(fromPc, s);

    recompute_material_and_psq();
    st->key ^= Zobrist::psq[pc][s] ^ Zobrist::psq[fromPc][s]
             ^ Zobrist::psqDark[pc][restoredCount - 1];
    st->checkersBB = checkers_to(~sideToMove, square<KING>(sideToMove));
    set_check_info(st);
...
void Position::recompute_material_and_psq() {
    ...
    for (Bitboard b = pieces(); b; )
    {
        ...
        if (Darkof(pc) == UNKNOWN)
        {
            ...
            int signedEg = c == BLACK ? -eg : eg;
            File f = File(edge_distance(file_of(s)));
            if (f > FILE_E) --f;
            st->material[c] += eg;
            psq += make_score(signedEg, signedEg) + PSQT::psqCap[rank_of(s)][f];
        }
        else
        {
            st->material[c] += PieceValue[MG][pc];
            psq += PSQT::psq[pc][s];
        }
    }
}
...
bool Position::getDark(StateInfo& newSt, int& typecount, bool& isDarkDepth) {
    ...
    if (st->darkSquare == SQ_NONE)
        return false;

    // setDark() undoes the previous candidate before the next call.  The
    // non-zero darkTypes field therefore means that a candidate is currently
    // installed and should not be installed twice.
    if (st->darkTypes != 0)
        return false;

    Color us = color_of(st->darkPiece);
    constexpr int CandidateCount = int(sizeof(DarkCandidateTypes) / sizeof(DarkCandidateTypes[0]));
    while (st->darkTypeIndex < CandidateCount)
    {
        PieceType pt = DarkCandidateTypes[st->darkTypeIndex++];
        int count = restPieces[us].countType(pt);
        if (!count)
            continue;

        Piece candidate = make_piece(us, pt);
        do_flip(st->darkSquare, candidate);
        st->darkTypes = int(pt) + 1;
        typecount = count;
        isDarkDepth = st->darkDepth >= MAXDARKDEPTH;
        return true;
    }

    st->darkSquare = SQ_NONE;
    st->darkPiece = NO_PIECE;
    return false;
}

void Position::setDark() {
    if (st->darkSquare == SQ_NONE || st->darkTypes == 0)
        return;

    undo_flip(st->darkSquare, st->darkPiece);
    st->darkTypes = 0;
}
...
bool Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  st = &newSt;
  st->move = m;
  // These fields describe only a temporary identity-expansion branch and are
  // not part of the ordinary copied state prefix.  Never inherit a parent's
  // installed candidate into a normal child node.
  st->darkPiece = NO_PIECE;
  st->darkSquare = SQ_NONE;
  st->darkTypeIndex = 0;
  st->darkTypes = 0;
  ...
  if (captured)
  {
      ...
      remove_piece(capsq);
      // The real identity is already encoded by the dark piece on the
      // destination square.  Do not trust optional UCI suffixes here: a dark
      // move never reveals the moving piece.  The restPieces list contains
      // only identities which have not yet been assigned to board dark
      // pieces, so capturing a board dark piece must not remove anything from
      // that list.
  ...
  // A dark move is searched once for every still-available identity.  The
  // board remains hidden between branches; getDark() temporarily installs one
  // candidate identity and setDark() restores it.
  if (Darkof(pc) == UNKNOWN)
  {
      st->darkPiece = pc;
      st->darkSquare = to;
...
void Position::do_null_move(StateInfo& newSt) {
  ...
  // A null move must never inherit a temporary identity-expansion branch from
  // the parent state; these fields are only valid right after a dark move.
  st->darkPiece = NO_PIECE;
  st->darkSquare = SQ_NONE;
  st->darkTypeIndex = 0;
  st->darkTypes = 0;
