...
namespace {
  ...
  ExtMove* generate_all(const Position& pos, ExtMove* moveList) {
    ...
    Bitboard target  = Type == PSEUDO_LEGAL ? ~pos.pieces(Us)
                     : Type == CAPTURES     ? pos.pieces(~Us, PAWN) | pos.pieces(~Us, ADVISOR)
                                             | pos.pieces(~Us, ADVISOR_B) | pos.pieces(~Us, BISHOP)
                                             | pos.pieces(~Us, KNIGHT) | pos.pieces(~Us, ROOK)
                                             | pos.pieces(~Us, CANNON)
                                            : ~pos.pieces(); // QUIETS || QUIET_CHECKS

    // Capturing a king is not a legal move. The position code assumes
    // that both kings remain present after every move.
    ...
    {
    Bitboard b = attacks_bb<KING>(ksq) & ~pos.pieces(Us) & ~pos.pieces(~Us, KING);
        if (Type == QUIET_CHECKS)
...
ExtMove* generate<EVASIONS>(const Position& pos, ExtMove* moveList) {
    ...
    if (pt == ROOK || pt == CANNON)
        b &= (~line_bb(checksq, ksq) | pos.pieces(~us)) & ~pos.pieces(~us, KING);
    while (b)
        *moveList++ = make_move(ksq, pop_lsb(b));
    ...
    Bitboard target = (between_bb(ksq, checksq)) & ~pos.pieces(us) & ~pos.pieces(~us, KING);
