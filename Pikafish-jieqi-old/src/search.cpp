...
void Thread::search() {
  ...
  // two aggregation models produce different scores for the same position.
  // Clearing here (on the main thread, before workers start) avoids blocking
  // the UCI input loop that would otherwise happen inside setoption.
  if (mainThread)
  {
      bool worst = (Options["DarkSearchMode"] == "Worst");
      if (worst != lastDarkWorst)
      {
          lastDarkWorst = worst;
          TT.clear();
      }
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
             ...
             && ttValue < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            // The dark-chess capture generator can produce a pseudo-legal move
            // whose destination is empty (for example after hidden-piece state
            // expansion).  ProbCut requires a real capture; do not abort the
            // whole engine on this generator mismatch.
            if (move != excludedMove && pos.legal(move) && pos.capture(move))
            {
                ss->currentMove = move;
